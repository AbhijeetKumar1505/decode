...
class Verifier:
    def build_context(
        self, task_state: Any, last_observation: dict[str, Any] | None = None
    ) -> dict[str, Any]:
...
class ModelVerifier:
    ...
    def __init__(self, provider: Any, rule_verifier: Verifier | None = None) -> None:
    ...
    async def verify(
        ...
        try:
            ...
            return VerificationResult(
                valid=True, reasons=["reviewer unavailable; accepting"]
            )
        decision = parse_llm_response(raw)
        if "valid" not in decision:
            return VerificationResult(
                valid=True, reasons=["reviewer reply not parseable; accepting"]
            )
    ...
    @staticmethod
    def _review_prompt(task_state: Any, last_observation: dict[str, Any] | None) -> str:
        parts = [task_state.render_compact()]
        if last_observation is not None:
            parts.append(
                "Last observation: " + str(last_observation.get("summary", ""))[:400]
            )
