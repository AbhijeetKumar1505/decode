...
HOST_CAPABILITY_META = {
    ...
    "list_tools": (
        RiskLevel.READ,
        "List command-line tools installed on this host (optionally filtered by name); use it to discover what you can run via shell_command",
    ),
    ...
    "shell_command": (
        RiskLevel.WRITE,
        "Run any installed CLI tool by its command line (policy-checked, per-command risk-classified, scoped, audited); if the tool is absent the result reports it",
    ),
    "host_session": (
        RiskLevel.WRITE,
        "Run a sequence of commands in a stateful session",
    ),
    "session_open": (
        RiskLevel.READ,
    ...
    "session_close": (
        RiskLevel.READ,
        "Close the persistent session and return its transcript summary",
    ),
}
