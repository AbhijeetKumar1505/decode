...
class HostSession:
    def __init__(
        ...
        scope: Optional[FilesystemScope] = None,
        cwd: Optional[str] = None,
        env: Optional[Dict[str, str]] = None,
    ) -> None:
        self._policy = policy
        self._scope = scope
        self.cwd = str(Path(cwd).expanduser().resolve(strict=False)) if cwd else os.getcwd()
    ...
    def run(self, argv: List[str]) -> Dict[str, Any]:
        ...
        if argv and argv[0] == "cd":
            ...
            if self._scope is not None and not self._scope.allows(resolved, write=False):
        ...
        self.transcript.append({"command": [str(a) for a in argv], "cwd": self.cwd, "result": result})
        return result
