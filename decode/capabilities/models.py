...
_ARGUMENT_SCHEMAS: Dict[str, Dict[str, CapabilityArgument]] = {
    ...
    "file_write": {
        ...
        "content": CapabilityArgument(type=ArgumentType.STRING, required=True, max_length=65536),
    },
    "file_edit": {
        "path": CapabilityArgument(type=ArgumentType.PATH, required=True),
        "old": CapabilityArgument(type=ArgumentType.STRING, required=True, max_length=65536),
        "new": CapabilityArgument(type=ArgumentType.STRING, required=True, max_length=65536),
    },
    "file_fetch": {
    ...
    "process_list": {},
    "process_kill": {"pid": CapabilityArgument(type=ArgumentType.INTEGER, required=True, minimum=1)},
    "service_status": {"name": CapabilityArgument(type=ArgumentType.STRING, required=True)},
    "service_control": {
        "name": CapabilityArgument(type=ArgumentType.STRING, required=True),
        "action": CapabilityArgument(type=ArgumentType.ENUM, choices=["start", "stop", "restart"], required=True),
    },
    "shell_command": {"command": CapabilityArgument(type=ArgumentType.STRING, required=True, max_length=8192)},
    "host_session": {"commands": CapabilityArgument(type=ArgumentType.STRING, required=True, max_length=16384)},
}
_RESULT_SCHEMA = {
            ...
            if name not in params or params[name] in (None, ""):
                if argument.required:
                    alternatives_satisfied = (
                        name == "target"
                        and any(params.get(key) for key in ("url", "domain"))
                    )
                    if not alternatives_satisfied:
                        raise ValueError(f"missing required normalized argument: {name}")
    ...
    def _validate_port_range(name: str, value: str) -> None:
        for segment in value.split(","):
            ...
            if not all(bound.isdigit() for bound in bounds):
                raise ValueError(f"normalized argument '{name}' has an invalid port range")
            ports = [int(bound) for bound in bounds]
            if any(port < 1 or port > 65535 for port in ports):
                raise ValueError(f"normalized argument '{name}' has a port outside 1-65535")
...
CAPABILITIES: Dict[str, CapabilitySpec] = {
    ...
    "file_read": CapabilitySpec(
        name="file_read", description="Read a file within the authorized filesystem scope",
        category="host", risk=RiskLevel.READ, kind="internal", target_required=False),
    "file_list": CapabilitySpec(
        name="file_list", description="List a directory within the authorized scope",
        category="host", risk=RiskLevel.READ, kind="internal", target_required=False),
    "file_search": CapabilitySpec(
        name="file_search", description="Search files for a pattern within scope",
        category="host", risk=RiskLevel.READ, kind="internal", target_required=False),
    "file_write": CapabilitySpec(
        name="file_write", description="Write a file within the writable scope",
        category="host", risk=RiskLevel.WRITE, kind="internal", target_required=False),
    "file_edit": CapabilitySpec(
        name="file_edit", description="Replace text in a file within the writable scope",
        category="host", risk=RiskLevel.WRITE, kind="internal", target_required=False),
    "file_fetch": CapabilitySpec(
        name="file_fetch", description="Copy or stage a file between scoped paths",
        category="host", risk=RiskLevel.WRITE, kind="internal", target_required=False),
    "list_tools": CapabilitySpec(
        name="list_tools", description="List command-line tools installed on this host (optionally filtered)",
        category="host", risk=RiskLevel.READ, kind="internal", target_required=False),
    "process_list": CapabilitySpec(
        name="process_list", description="List running processes",
        category="host", risk=RiskLevel.READ, kind="internal", target_required=False),
    "process_kill": CapabilitySpec(
        name="process_kill", description="Terminate a process by PID",
        category="host", risk=RiskLevel.DESTRUCTIVE, kind="internal", target_required=False),
    "service_status": CapabilitySpec(
        name="service_status", description="Query a system service state",
        category="host", risk=RiskLevel.READ, kind="internal", target_required=False),
    "service_control": CapabilitySpec(
        name="service_control", description="Start, stop, or restart a system service",
        category="host", risk=RiskLevel.DESTRUCTIVE, kind="internal", target_required=False),
    "shell_command": CapabilitySpec(
        name="shell_command", description="Run a policy-checked argument-vector command",
        category="host", risk=RiskLevel.WRITE, kind="internal", target_required=False),
    "host_session": CapabilitySpec(
        name="host_session", description="Run a sequence of commands in a stateful session",
        category="host", risk=RiskLevel.WRITE, kind="internal", target_required=False),
}
