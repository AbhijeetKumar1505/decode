...
class HostAgent(Agent):
    ...
    def capabilities(self) -> List[str]:
        return [
            ...
            "file_write", "file_edit", "file_fetch",
            "list_tools",
            "process_list", "process_kill",
            "service_status", "service_control",
            "shell_command", "host_session",
        ]
    ...
    async def execute_internal(self, node: Any, context: dict) -> AgentResult:
        ...
        try:
            ...
            return self._result(node.capability, {"ok": False, "error": f"{type(exc).__name__}: {exc}"})
        return self._result(node.capability, result)

    def _dispatch(self, capability: str, params: dict, scope: FilesystemScope, policy: CommandPolicy | None, stdin: str | None = None) -> dict:
        ...
        if capability == "file_search":
            return ops.file_search(params["root"], params["pattern"], scope, glob=params.get("glob", "*"))
        ...
        if capability == "list_tools":
            return ops.list_tools(params.get("query", ""), int(params.get("limit", 400)))
    ...
    def _shell(self, params: dict, policy: CommandPolicy | None, stdin: str | None = None) -> dict:
        if policy is None:
            return {"ok": False, "error": "no command policy in scope; shell command denied"}
        ...
        if policy.classify(argv) is _HostRisk.DESTRUCTIVE:
            return {"ok": False, "error": "command classified DESTRUCTIVE; not permitted via shell_command"}
    ...
    def _session(self, commands: str, scope: FilesystemScope, policy: CommandPolicy | None) -> dict:
        if policy is None:
            return {"ok": False, "error": "no command policy in scope; session denied"}
        try:
            steps = json.loads(commands)
        except (json.JSONDecodeError, TypeError):
            return {"ok": False, "error": "commands must be a JSON list of argument vectors"}
        ...
        for step in steps:
            ...
            if argv and policy.classify(argv) is _HostRisk.DESTRUCTIVE:
                session.transcript.append({"command": argv, "cwd": session.cwd,
    ...
    def _result(self, capability: str, data: dict) -> AgentResult:
        ...
        return AgentResult(
            ...
            success=ok,
            summary=summary[:300],
            output="" if not ok else json.dumps({k: v for k, v in data.items() if k not in ("ok", "error")})[:2000],
            error=error,
