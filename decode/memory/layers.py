...
SENSITIVE_TYPES = {"credential", "token", "cookie", "jwt", "api_key", "password", "secret"}
...
class ProjectMemory:
    ...
    def __init__(self, store: SessionStore, project_id: Optional[str] = None) -> None:
        self._store = store
    ...
    def remember(
        ...
        return self._store.add_artifact(
            type=type, key=key, value=value, session_id=session_id,
            project_id=self.project_id, sensitive=sensitive,
        )

    def recall(self, type: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    def exact(self, query: str) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            if normalized in item["key"].casefold() or normalized in item["value"].casefold()
        ]
...
class KnowledgeMemory:
    ...
    def learn(self, name: str, description: str = "", type: str = "finding", **properties: Any) -> str:
        from ..knowledge.graph import KnowledgeNode
        node = KnowledgeNode(type=type, name=name, description=description, properties=properties)
...
class ProjectKnowledgeMemory:
    ...
    def relate(
        ...
        return self._store.add_project_knowledge_edge(
            self.project_id, source_id, target_id, relationship,
...
class HybridRetriever:
    ...
    def __init__(
        ...
        semantic_search: Optional[Callable[[str], List[Dict[str, Any]]]] = None,
    ) -> None:
    ...
    def retrieve(self, query: str) -> List[Dict[str, Any]]:
        ...
        for artifact in self._project.exact(query):
            results.append({
                ...
                "record": artifact,
                "provenance": {"artifact_id": artifact["id"], "sensitive": bool(artifact["sensitive"])},
            })
        ...
        if self._semantic_search is not None:
            for item in self._semantic_search(query):
                results.append({
                    "source": "semantic",
                    "record": item,
                    "provenance": {"verification": "unverified", "retrieval": "optional_semantic"},
                })
        return results
...
class MemoryManager:
    ...
    def __init__(
        ...
        self.knowledge = (
            ...
            ProjectKnowledgeMemory(store, project_id) if project_id else KnowledgeMemory()
        )
        self.retriever = (
            HybridRetriever(self.project, self.knowledge, semantic_search)
            if isinstance(self.knowledge, ProjectKnowledgeMemory) else None
        )
    ...
    def capture_host(self, host: str) -> Optional[str]:
        if not host:
            return None
